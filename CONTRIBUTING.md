...
please read [the GitHub official documentation][github-quickstart].
...
This project uses [Gradle] as dependency manager and Groovy DSL
...
[here is a tutorial from Gradle official][Gradle].
...
with the help of [Architectury Loom],
a framework for multi-platform modding.
Derivatives from [Fabric Loom].
...
you can ask in [their Discord server][architectury-discord] instead of searching for documentation,
...
Then the core framework we heavily depend on - [SpongePowered Mixin][mixin],
...
reading [the tutorial by Fabric][fabric-mixin] and [the official wiki][mixin].
...
you can ask in `mod-dev` channels of [the Fabric Discord server][fabric-discord]
or the `mixin` channel of [NeoForge Discord server][neoforge-discord],
...
we need to [read the original code][decompilation].
Thanks to the [Yarn] tool we can read the source code
...
If you'd like
to know some terms of Minecraft project structure like [`client side` vs `server side`][sides],
...
set generated source jar as compiled jar's source according to the
`Generating Minecraft Sources` section of the [tutorial][fabric-setup].

Now you can explore the project.

For IntelliJ, the Loom will generate some [Run Configurations] like `Minecraft Client (:fabric)`,
`Minecraft Server (:fabric)`, `Minecraft Client (:neoforge)`, `Minecraft Server (:neoforge)`,
you can tell the purpose of each configuration by their name,
for example, `Minecraft Client (:fabric)` is for starting the client side game on Fabric.
...
* `gradle`: Mainly contains Gradle wrapper configuration,
  Gradle wrapper's version is passively upgraded as Loom's version is upgraded.
* `doc`: Project documentations.
* `build.gradle`: Root Gradle build script of this Gradle managing project.
  ([Gradle doc about `build.gradle` file][build.gradle])
* `gradle.properties`: Configuration about mod and dependencies versioning.
* `settings.gradle`: Defines that this project has three subprojects:
  `common`, `fabric`, `neoforge`. ([Gradle doc about `settings.gradle` file][settings.gradle])
* `common`: Gradle subproject, Platform-independent code and configurations, normal Java project structure.
    * `build.gradle`: Gradle build script of `common` subproject, it has developing dependencies like
      `lombok`, `log4j`, `junit`.
    ...
    * `src/main/resources`
        ...
        * `minecraft_access-common.mixins.json`: Mixin configuration that tells the Mixin framework where to find Mixin
          classes under `common` directory.
        * `assets/minecraft_access`: Contains custom resource files of this mod,
          structured in the [Minecraft Resource Pack] format.
...
* `fabric`: Gradle subproject, Fabric-dependent mod code.
    ...
    * `src/main/resources`:
        ...
        * `fabric.mod.json`: [Fabric mod configuration][fabric.mod.json].
          You can see we refer both Mixin config files under `common` and `fabric` subprojects in this file.
* `neoforge`: Gradle subproject, NeoForge-dependent mod code.
    ...
    * `src/main/resources`:
        ...
        * `META-INF/neoforge.mods.toml`: [NeoForge mod configuration][neoforge.mods.toml].
...
The [tick] mechanism constantly triggers the other type of logic.
This is achieved by adding custom function invoking logic into original game's tick processing method in
platform-dependent code, which is called every tick.
Different platforms implement different ways to achieve this.
The `MainClass.clientTickEventsMethod()` is invoked when platform tick event is fired,
then it calls every execution function inside this method, each feature will check if its condition is met,
if so, it will execute their particular logic, or it will directly return as skipping this tick.
This execution type is suitable for implementing new features that not exist in the original game,
like `Camera Controls` that rotating the player view with keyboard.
...
Applying less mock and embracing programming strategies like [Functional Core, Imperative Shell],
[Red - Green - Refactor]
and more described in [this article][mocking].
...
* For every PR and building, [`linting` workflow][linting] will be triggered for running the test suite against current
  code.
* The [`build` workflow][build] is responsible for building all platforms mod jars, you can manually dispatch it in
  GitHub Action Page.
* When a new version is ready and merged, we'll manually run the [`release` workflow][release] to create a new
  pre-release in GitHub Release, after one-week exposure, we'll run this workflow against this version again to change
  the GitHub release to formal state, and publish mod to CurseForge and Modrinth.

## Developing Tips

* You'd like to enable this mod's `Other Configurations - Debug Mode` configuration to let the mod print debug level
  logs.
* When running the game with IDE, the main directory is `{platform directory}/run`
  (`.minecraft` in production environment), you can see familiar directories like `config`, `mods`, `saves` under it.
* You can run `gradlew {platform}:build` to build mod for a specific platform
  (when root Gradle `build` task fails for some reason), built mod jars are under `{platform directory}/build/libs`,
...
make a new issue, or talk in the `mc-dev` channel of our [Discord server][Discord].
...
(praise [the freewheeling open source community][cathedral-bazaar]!),
...
[github-quickstart]: https://docs.github.com/en/get-started/quickstart/contributing-to-projects
...
[architectury-discord]: https://discord.architectury.dev/
[mixin]: https://github.com/SpongePowered/Mixin/wiki
[fabric-mixin]: https://fabricmc.net/wiki/tutorial:mixin_introduction
[fabric-discord]: https://discord.com/invite/v6v4pMv
[neoforge-discord]: https://discord.com/invite/neoforged
[decompilation]: https://fabricmc.net/wiki/tutorial:reading_mc_code
[Yarn]: https://fabricmc.net/wiki/documentation:yarn
[sides]: https://fabricmc.net/wiki/tutorial:side
[architectury-idea]: https://plugins.jetbrains.com/plugin/16210-architectury
[Minecraft Development]: https://plugins.jetbrains.com/plugin/8327-minecraft-development
[fabric-setup]: https://fabricmc.net/wiki/tutorial:setup
[Run Configurations]: https://www.jetbrains.com/help/idea/run-debug-configuration.html
[debugging mixin]: https://fabricmc.net/wiki/tutorial:mixin_export
[build.gradle]: https://docs.gradle.org/current/userguide/build_file_basics.html
[settings.gradle]: https://docs.gradle.org/current/userguide/settings_file_basics.html
[Minecraft Resource Pack]: https://minecraft.wiki/w/Resource_pack
[fabric.mod.json]: https://fabricmc.net/wiki/documentation:fabric_mod_json
[neoforge.mods.toml]: https://docs.neoforged.net/docs/gettingstarted/modfiles/#neoforgemodstoml
...
[mocking]: https://www.amazingcto.com/mocking-is-an-antipattern-how-to-test-without-mocking/
[linting]: .github/workflows/linting.yml
[build]: .github/workflows/build.yml
[release]: .github/workflows/release.yml
[cathedral-bazaar]: http://www.catb.org/~esr/writings/cathedral-bazaar/
